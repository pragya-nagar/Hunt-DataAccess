...
{
    public partial class UpdateStoredProcedureAmountsDueRecalculation : Migration
    {
        protected override void Up(MigrationBuilder migrationBuilder)
        {
            var stageSchemaSQL = @"
                ...
                BEGIN
                    ...
                    UPDATE main.""Delinquency"" d SET
    		                ""IsLoan"" = FALSE,								 /* Set loan delinquency to lien */
    		                ""EventId"" = COALESCE(d.""EventId"", res.event_id), /* Set loan delinquency to lien */
                            ""Amount"" = COALESCE(res.amount, d.""Amount""),
                            ""LTVPercent"" = COALESCE(res.ltv_percent, d.""LTVPercent""), 
                            ""RUAmount"" = COALESCE(res.ru_amount, d.""RUAmount""),
                            ""RULTVPercent"" = COALESCE(res.ru_ltv_percent, d.""RULTVPercent""),
                    ...
                    FROM res
                    WHERE d.""Id"" = res.delinquency_id
                        AND (  d.""EventId"" IS DISTINCT FROM COALESCE(res.event_id, d.""EventId"")  
    		                OR d.""Amount"" IS DISTINCT FROM COALESCE(res.amount, d.""Amount"")
                            OR d.""LTVPercent"" IS DISTINCT FROM COALESCE(res.ltv_percent, d.""LTVPercent"")
                            OR d.""RUAmount"" IS DISTINCT FROM COALESCE(res.ru_amount, d.""RUAmount"")
                            OR d.""RULTVPercent"" IS DISTINCT FROM COALESCE(res.ru_ltv_percent, d.""RULTVPercent""));
                    ...
                    INSERT INTO main.""Delinquency""(""Id"", ""PropertyId"", ""DelinquencyTaxYear"", ""Amount"", ""LTVPercent"", ""RUAmount"", ""RULTVPercent"", ""IsLoan""
    								                , ""CreatedOn"", ""CreatedById"", ""ModifiedOn"", ""ModifiedById"")		
                    SELECT uuid_generate_v4() as id, d.property_id, d.delinquency_tax_year, d.amount, d.ltv_percent, d.ru_amount, d.ru_ltv_percent, true
    								                , CURRENT_TIMESTAMP AT TIME ZONE 'UTC', p_system_user_id, CURRENT_TIMESTAMP AT TIME ZONE 'UTC', p_system_user_id
                    FROM d
            ...
            var systemSchemaSQL = @"
                ...
                /* DROP/CREATE get_UID FUNCTION */
                DROP FUNCTION IF EXISTS system.get_uid();

                CREATE OR REPLACE FUNCTION system.get_uid() RETURNS UUID AS $$ 
                SELECT CAST(COALESCE(NULLIF(current_setting('Synergy.UID', true), ''), '00000000-0000-0000-0000-000000000001') AS UUID);
                $$ LANGUAGE SQL;


                /* DROP/CREATE get_operation_id FUNCTION */
                DROP FUNCTION IF EXISTS system.get_operation_id();

                CREATE OR REPLACE FUNCTION system.get_operation_id()  RETURNS UUID AS $$ 
                SELECT CAST(COALESCE(NULLIF(current_setting('Synergy.OperationID', true), ''), NULLIF(set_config('Synergy.OperationID', CAST(uuid_generate_v4() AS VARCHAR(36)), false), '')) AS UUID);
                $$ LANGUAGE SQL;


                /* DROP/CREATE create_audit_triggers FUNCTION */
                DROP FUNCTION IF EXISTS system.create_audit_triggers(VARCHAR, VARCHAR, VARCHAR);

                CREATE OR REPLACE FUNCTION system.create_audit_triggers(p_schema_name character varying, p_history_table_suffix character varying)
                 RETURNS void
                 LANGUAGE plpgsql
                AS $function$
                DECLARE 	
                ...
                BEGIN
                    ...
                    LOOP
                        ...
                        THEN
    		                ...
    		                SELECT concat(string_agg(quote_ident(s.column_name), ','), ',', quote_ident('InsertedOn'), ',', quote_ident('InsertedBy'), ',', quote_ident('OperationId')) 
    		                INTO s_insert_columns_list
			                FROM information_schema.columns s
				                INNER JOIN information_schema.columns d ON d.table_schema = p_schema_name
				                AND d.table_name = CONCAT(r_table.table_name, p_history_table_suffix) AND d.column_name = s.column_name                                      
			                WHERE s.table_schema = p_schema_name
			                ...
			                EXECUTE CONCAT('CREATE OR REPLACE FUNCTION ',  p_schema_name, '.audit_', lower(r_table.table_name),'_update() ', chr(10)
				                ...
				                ,'	VALUES(', s_values_columns_list, ',', 'clock_timestamp(), system.get_uid(), system.get_operation_id());', chr(10)
				                ,'END IF;', chr(10)
				                ,'RETURN NEW;', CHR(10)
				                ,'END;', chr(10)
				                ,'$BODY$;', chr(10), chr(10)
			                ...
			                EXECUTE CONCAT('CREATE OR REPLACE FUNCTION ',  p_schema_name, '.audit_', lower(r_table.table_name),'_delete() ', chr(10)
				                ...
				                ,'BEGIN', chr(10)
				                ,'	INSERT INTO ', p_schema_name, '.""', r_table.table_name, p_history_table_suffix, '"" (', s_insert_columns_list, ')', chr(10)
				                ,'	VALUES(', REPLACE(s_values_columns_list, 'OLD.""DeletedOn""', 'clock_timestamp()'), ',', 'clock_timestamp(), system.get_uid(), system.get_operation_id());', chr(10)
				                ,'RETURN NEW;', CHR(10)
				                ,'END;', chr(10)
				                ,'$BODY$;', chr(10), chr(10)
            ...
            migrationBuilder.Sql(systemSchemaSQL, true);
