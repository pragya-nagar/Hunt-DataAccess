...
{
    public partial class ChangeEtlToSupportMultipleDelinquency : Migration
    {
                ...
                BEGIN
	                ...
	                (
		                SELECT DISTINCT ON (p.""Id"", psd.delinquency_tax_year)
			                 p.""Id"" as property_id
			                ,psd.delinquency_tax_year
			                ,sum(amount_due) OVER (PARTITION BY p.""Id"", psd.delinquency_tax_year ORDER BY p.""Id"", psd.delinquency_tax_year) AS amount
			                ,ru_amount
			                ,ru_ltv_percent
			                ,ltv_percent
		                FROM stage.synergy_source_data psd
	                ...
	                (
		                SELECT DISTINCT ON (p.""Id"", psd.delinquency_tax_year)
			                ...
			                ,e.""Id"" as event_id
			                ,delinquency_tax_year
			                ,sum(amount_due) OVER (PARTITION BY p.""Id"", e.""Id"", delinquency_tax_year ORDER BY p.""Id"", e.""Id"", delinquency_tax_year) AS amount
			                ,ru_amount
			                ,ru_ltv_percent
			                ,ltv_percent
		                FROM stage.synergy_source_data psd
