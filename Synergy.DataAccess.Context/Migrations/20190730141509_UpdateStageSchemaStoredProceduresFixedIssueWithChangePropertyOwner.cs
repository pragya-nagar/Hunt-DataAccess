...
{
    public partial class UpdateStageSchemaStoredProceduresFixedIssueWithChangePropertyOwner : Migration
    {
        protected override void Up(MigrationBuilder migrationBuilder)
        {
            var stageSchemaSQL = @"
                ...
                /* drop stage.batch_to_process table   */
                DROP TABLE IF EXISTS stage.batch_to_process;

                /* create stage.batch_to_process table */
                CREATE TABLE stage.batch_to_process (
                ...
                /* DROP load_main_lead FUNCTION */
                DROP FUNCTION IF EXISTS stage.load_main_lead_batch(p_batch_id integer);
                /* CREATE load_main_lead FUNCTION */
                CREATE OR REPLACE FUNCTION stage.load_main_lead_batch(p_batch_id integer)
                ...
                BEGIN
                    ...
                    (
                        ...
                        WHERE psd.batch_id = p_batch_id
			                AND lead_bk IS NOT NULL
                ...
                /* DROP load_main_property FUNCTION */
                DROP FUNCTION IF EXISTS stage.load_main_property_batch(p_batch_id integer);
                /* CREATE load_main_property FUNCTION */
                CREATE OR REPLACE FUNCTION stage.load_main_property_batch(p_batch_id integer)
                 RETURNS TABLE(updated_count numeric, inserted_count numeric)
                 LANGUAGE plpgsql
                AS $function$
                DECLARE 
                ...
                BEGIN
                    ...
                    (
                        ...
                        FROM stage.synergy_source_data psd
                            ...
                            LEFT JOIN main.""GeneralLandUseCode"" gluc ON LOWER(psd.general_land_use_code) = LOWER(gluc.""Name"")
                            LEFT JOIN main.""InternalLandUseCode"" iluc ON LOWER(psd.internal_land_use_code) = LOWER(iluc.""Description"")
                        WHERE psd.batch_id = p_batch_id
        	                AND psd.property_bk IS NOT NULL
                    )
                    UPDATE main.""Property"" p SET 
                            ""LeadId"" = COALESCE(pr.lead_id, ""LeadId"")
                            ,""CADId"" = COALESCE(pr.cad_id, ""CADId"")
                            ,""TAXId"" = COALESCE(pr.tax_id, ""TAXId"")
                            ,""FolioId"" = COALESCE(pr.folio_id, ""FolioId"")
                    ...
                    WHERE pr.rn = 1
                        ...
                        AND (	""LeadId"" IS DISTINCT FROM COALESCE(pr.lead_id, ""LeadId"") OR
        		                ""CADId"" IS DISTINCT FROM COALESCE(pr.cad_id, ""CADId"") OR
        		                ""TAXId"" IS DISTINCT FROM COALESCE(pr.tax_id, ""TAXId"") OR
        		                ""FolioId"" IS DISTINCT FROM COALESCE(pr.folio_id, ""FolioId"") OR
                    ...
                    (
                        SELECT property_bk, l.""Id"" as lead_id, county_id, parcel_id, cad_id, tax_id, folio_id, 
                            ...
                            SUM(CASE WHEN event_batch_id IS NULL THEN COALESCE(amount_due, 0) ELSE 0 END) OVER(PARTITION BY property_bk ORDER BY property_bk) AS total_amount	,
                            SUM(CASE WHEN event_batch_id IS NULL THEN COALESCE(amount_due, 0) ELSE 0 END) OVER(PARTITION BY property_bk, delinquency_tax_year ORDER BY property_bk, delinquency_tax_year) AS last_year_due,
                ...
                /* DROP load_main_property_valuation FUNCTION */
                DROP FUNCTION IF EXISTS stage.load_main_property_valuation_batch(p_batch_id integer);
                /* CREATE load_main_property_valuation FUNCTION */
                CREATE OR REPLACE FUNCTION stage.load_main_property_valuation_batch(p_batch_id integer)
                 RETURNS TABLE(updated_count numeric, inserted_count numeric)
                 LANGUAGE plpgsql
                AS $function$
                    DECLARE 
                ...
                /* DROP load_main_property_supplemental_event_data FUNCTION */
                DROP FUNCTION IF EXISTS stage.load_main_property_supplemental_event_data_batch(p_batch_id integer);
                /* CREATE load_main_property_supplemental_event_data FUNCTION */
                CREATE OR REPLACE FUNCTION stage.load_main_property_supplemental_event_data_batch(p_batch_id integer)
                 RETURNS TABLE(updated_count numeric, inserted_count numeric)
                 LANGUAGE plpgsql
                AS $function$
                    DECLARE 
                ...
                BEGIN
                    ...
                    (
                        SELECT d.""Id"" as delinquency_id, last_sale_date, last_sale_amount, mortgage_lender_1, 
                            ...
                            advertisement_batch, advertisement_number, assessor_url, treasurer_url, gis_url,
                    ...
                    UPDATE main.""PropertySupplementalEventData"" psed SET 
                            ...
                            ,""GisURL"" = COALESCE(sd.gis_url, psed.""GisURL"")
                            ,""ModifiedOn"" = CURRENT_TIMESTAMP AT TIME ZONE 'UTC'
                            ,""ModifiedById"" = p_system_user_id
                    FROM sd
                    WHERE sd.rn = 1
                        ...
                        AND (	   psed.""LastSaleDate"" IS DISTINCT FROM COALESCE(sd.last_sale_date, psed.""LastSaleDate"")
                                ...
                                OR psed.""GisURL"" IS DISTINCT FROM COALESCE(sd.gis_url, psed.""GisURL""));
                    ...
                    INSERT INTO main.""PropertySupplementalEventData""(""Id"", ""DelinquencyId"", ""PropertyId"", ""LastSaleDate"", ""LastSaleAmount"", 
                                ...
                                ""RecentBuyerRate"", ""AdvertisementBatch"", ""AdvertisementNumber"", ""AssessorURL"", ""TreasurerURL"", ""GisURL"",
                                ""CreatedOn"", ""CreatedById"", ""ModifiedOn"", ""ModifiedById"")		
                    SELECT uuid_generate_v4() as id, sd.delinquency_id, sd.property_id, sd.last_sale_date, sd.last_sale_amount, 
                                ...
                                sd.recent_buyer_rate, sd.advertisement_batch, sd.advertisement_number, sd.assessor_url, sd.treasurer_url, sd.gis_url,
                                CURRENT_TIMESTAMP AT TIME ZONE 'UTC', p_system_user_id, CURRENT_TIMESTAMP AT TIME ZONE 'UTC', p_system_user_id
                    FROM sd
                    WHERE sd.rn = 1
                ...
                /* DROP load_main_delinquency FUNCTION */
                DROP FUNCTION IF EXISTS stage.load_main_delinquency_batch(p_batch_id integer);
                /* CREATE load_main_delinquency FUNCTION */
                CREATE OR REPLACE FUNCTION stage.load_main_delinquency_batch(p_batch_id integer)
                 RETURNS TABLE(updated_count numeric, inserted_count numeric)
                 LANGUAGE plpgsql
                AS $function$
                    DECLARE 
                        p_system_user_id UUID;
                        p_other_entity_type_id int4;
                        p_ucnt numeric;
                        p_icnt numeric;
                        p_tucnt numeric;
                        p_ticnt numeric;
                BEGIN
                    ...
                    /* Get Other entity type id */
                    SELECT ""Id"" INTO p_other_entity_type_id FROM main.""CollectingEntityType"" WHERE ""Name"" = 'Other';
                    ...
                    (
                        SELECT DISTINCT ON (d.""Id"", psd.delinquency_tax_year) 
                            ...
                            ,ltv_percent
                        FROM stage.synergy_source_data psd
                            LEFT JOIN main.""Event"" e ON e.""EventNumber"" = psd.event_batch_id
                            INNER JOIN main.""Property"" p ON p.""PropertyBK"" = psd.property_bk
                            INNER JOIN main.""Delinquency"" d ON COALESCE(d.""EventId"", '00000000-0000-0000-0000-000000000001') = COALESCE(e.""Id"", '00000000-0000-0000-0000-000000000001')
            	                AND d.""PropertyId"" = p.""Id"" AND d.""DelinquencyTaxYear"" = psd.delinquency_tax_year
                        WHERE psd.batch_id = p_batch_id
			                AND psd.property_bk IS NOT NULL
                    ...
                    GET DIAGNOSTICS p_tucnt = ROW_COUNT;
   

                    /* Insert new Delinquency */	
                    WITH d AS
                    (
                        SELECT DISTINCT ON (e.""Id"", p.""Id"", psd.delinquency_tax_year)
                             p.""Id"" as property_id
                            ,e.""Id"" as event_id
                            ,delinquency_tax_year
                        ...
                        FROM stage.synergy_source_data psd
                            ...
                            LEFT JOIN main.""Event"" e ON e.""EventNumber"" = psd.event_batch_id
                        WHERE psd.batch_id = p_batch_id
        	                AND (e.""EventNumber"" IS NOT NULL OR psd.event_batch_id IS NULL)
			                AND psd.property_bk IS NOT NULL
                            AND psd.delinquency_tax_year IS NOT NULL	
                    )
                    INSERT INTO main.""Delinquency""(""Id"", ""PropertyId"", ""EventId"", ""DelinquencyTaxYear"", ""Amount"", ""LTVPercent"", ""RUAmount"", ""RULTVPercent""
    								                , ""IsLoan""
    								                , ""CreatedOn"", ""CreatedById"", ""ModifiedOn"", ""ModifiedById"")		
                    SELECT uuid_generate_v4() as id, d.property_id, d.event_id, d.delinquency_tax_year, d.amount, d.ltv_percent, d.ru_amount, d.ru_ltv_percent
    								                , CASE WHEN d.event_id IS NULL THEN TRUE ELSE FALSE END
    								                , CURRENT_TIMESTAMP AT TIME ZONE 'UTC', p_system_user_id, CURRENT_TIMESTAMP AT TIME ZONE 'UTC', p_system_user_id
                    FROM d
                    WHERE NOT EXISTS ( SELECT 1 FROM main.""Delinquency"" dl 
		                                WHERE COALESCE(dl.""EventId"", '00000000-0000-0000-0000-000000000001') = COALESCE(d.event_id, '00000000-0000-0000-0000-000000000001')
		                	                AND dl.""PropertyId"" = d.property_id 
			                                AND dl.""DelinquencyTaxYear"" = d.delinquency_tax_year
	                                 );

                    GET DIAGNOSTICS p_ticnt =  ROW_COUNT;


                    /* Update CollectingEntity */
                    WITH res AS
                    (
                        SELECT dl.""Id"" as delinquency_id, COALESCE(cet.""Id"", p_other_entity_type_id) as type_id, SUM(COALESCE(psd.amount_due, 0)) AS amount_due
                        FROM stage.synergy_source_data psd     
                            INNER JOIN main.""Property"" p ON p.""PropertyBK"" = psd.property_bk
                            LEFT JOIN main.""Event"" e ON e.""EventNumber"" = psd.event_batch_id
                            INNER JOIN main.""Delinquency"" dl ON dl.""PropertyId"" = p.""Id"" 
                                AND dl.""DelinquencyTaxYear"" = psd.delinquency_tax_year
                                AND COALESCE(dl.""EventId"", '00000000-0000-0000-0000-000000000001') = COALESCE(e.""Id"", '00000000-0000-0000-0000-000000000001') 
                            LEFT JOIN main.""CollectingEntityType"" cet ON LOWER(cet.""Name"") = LOWER(psd.collecting_entity)
                        WHERE psd.batch_id = p_batch_id
        	                AND (e.""Id"" IS NOT NULL OR psd.event_batch_id IS NULL)
			                AND psd.property_bk IS NOT null
		                GROUP BY dl.""Id"", COALESCE(cet.""Id"", p_other_entity_type_id)
                    )
                    ...
                    GET DIAGNOSTICS p_ucnt = ROW_COUNT;
                    p_tucnt = CASE WHEN p_tucnt >= p_ucnt THEN p_tucnt ELSE p_ucnt END;

                    /* Insert new CollectingEntity */
                    WITH res AS
                    (
                        SELECT dl.""Id"" as delinquency_id, COALESCE(cet.""Id"", p_other_entity_type_id) as type_id, SUM(COALESCE(psd.amount_due, 0)) AS amount_due
                        FROM stage.synergy_source_data psd     
                    ...
                    GET DIAGNOSTICS p_icnt =  ROW_COUNT;
                    p_ticnt = CASE WHEN p_ticnt >= p_icnt THEN p_ticnt ELSE p_icnt END;
   
   
                    RETURN QUERY
                    SELECT COALESCE(p_tucnt, 0), COALESCE(p_ticnt, 0);		
                ...
                /* DROP refresh_total_amounts FUNCTION */
                DROP FUNCTION IF EXISTS stage.refresh_total_amounts_batch(p_batch_id integer);
                /* CREATE refresh_total_amounts FUNCTION */
                CREATE OR REPLACE FUNCTION stage.refresh_total_amounts_batch(p_batch_id integer)
                 RETURNS TABLE(updated_count numeric, inserted_count numeric)
                 LANGUAGE plpgsql
                AS $function$
                    DECLARE 
                    ...
                    p_utotal = 0;
		                ...
		                WITH pr AS(
                                ...
                                WHERE d.""IsLoan"" = true
                	                AND EXISTS (SELECT 1 FROM main.""Property"" p
				                                ...
				                                WHERE sd.batch_id = p_batch_id AND p.""Id"" = d.""PropertyId"")
                            )
                        ...
                        WITH lead AS(
                                SELECT l.""Id"" AS ""LeadId""
	                                ,SUM(COALESCE(p.""TotalAmountDue"", 0)) AS ""TotalAmountDueProperties""
	                                ,SUM(COALESCE(pv.""AppraisedValue"", 0)) AS ""AppraisedValueOfProperties""
                                FROM main.""Lead"" l
                	                LEFT JOIN main.""Property"" p ON p.""LeadId"" = l.""Id""   
	                                LEFT JOIN main.""PropertyValuation"" pv ON pv.""PropertyId"" = p.""Id"" AND pv.""IsActive"" = true              
                                WHERE EXISTS (  SELECT 1 FROM 
								                (
									                SELECT p.""LeadId"" FROM main.""Property"" p
									                INNER JOIN stage.synergy_source_data sd ON p.""PropertyBK"" = sd.property_bk
									                WHERE sd.batch_id = p_batch_id 
									                UNION ALL
									                SELECT pa.""LeadId"" FROM main.""PropertyAudit"" pa
									                INNER JOIN stage.synergy_source_data sd ON pa.""PropertyBK"" = sd.property_bk
									                WHERE sd.batch_id = p_batch_id
								                ) ld WHERE ld.""LeadId"" = l.""Id""
							                )
                                GROUP BY l.""Id""  
                                )
                         UPDATE main.""Lead"" l
                ...
                /* DROP load_profile_delinquency FUNCTION */
                DROP FUNCTION IF EXISTS stage.load_profile_delinquency();
                /* DROP load_profile_delinquency FUNCTION */
                DROP FUNCTION IF EXISTS stage.load_profile_delinquency_batch(p_batch_id integer);
                /* CREATE load_profile_delinquency FUNCTION */
                CREATE OR REPLACE FUNCTION stage.load_profile_delinquency_batch(p_batch_id integer)
                 RETURNS TABLE(updated_count numeric, inserted_count numeric)
                 LANGUAGE plpgsql
                AS $function$
                    DECLARE 
                ...
                BEGIN
	                ...
	                INSERT INTO main.""EtlDelinquency""
	                SELECT DISTINCT 
		                  d.""EventId"" AS ""EventId"" 
	                     ,d.""Id"" AS ""DelinquencyId""
	                     ,CURRENT_TIMESTAMP AT TIME ZONE 'UTC' AS ""CreatedOn""
	                FROM stage.synergy_source_data psd
	                    INNER JOIN main.""Property"" p ON p.""PropertyBK"" = psd.property_bk
	                    INNER JOIN main.""Delinquency"" d ON d.""PropertyId"" = p.""Id"" 
	                        AND d.""DelinquencyTaxYear"" = psd.delinquency_tax_year
	                    LEFT JOIN main.""EtlDelinquency"" ed ON ed.""EventId"" = d.""EventId"" AND ed.""DelinquencyId"" = d.""Id""
	                WHERE psd.batch_id = p_batch_id
		                AND psd.property_bk IS NOT NULL
		                AND psd.delinquency_tax_year IS NOT NULL
	                    AND psd.event_batch_id IS NOT NULL
                    ...
                    RETURN QUERY
                    SELECT 0::numeric, COALESCE(p_inserted, 0);
                ...
                /* DROP create_new_county_batch FUNCTION */
                DROP FUNCTION IF EXISTS stage.create_new_county_batch(p_batch_id integer);
                /* CREATE create_new_county_batch FUNCTION */
                CREATE OR REPLACE FUNCTION stage.create_new_county_batch(p_batch_id integer)
                 RETURNS TABLE(updated_count numeric, inserted_count numeric)
                 LANGUAGE plpgsql
                AS $function$
                    DECLARE 
                        p_system_user_id UUID = '00000000-0000-0000-0000-000000000001';
                        p_icnt numeric = 0;
                BEGIN
	

                    INSERT INTO main.""County""(""StateId"", ""Name"", ""CreatedOn"", ""CreatedById"", ""ModifiedOn"", ""ModifiedById"")		
                    SELECT DISTINCT ON (s.""Id"", county_jurisdiction) s.""Id"", county_jurisdiction
    	                ,CURRENT_TIMESTAMP AT TIME ZONE 'UTC', p_system_user_id, CURRENT_TIMESTAMP AT TIME ZONE 'UTC', p_system_user_id        
                        FROM stage.synergy_source_data psd
        	                INNER JOIN main.""State"" s ON LOWER(s.""Abbreviation"") = LOWER(psd.property_state)
        	                LEFT JOIN main.""County"" c ON LOWER(c.""Name"") = LOWER(psd.county_jurisdiction)
                        WHERE psd.batch_id = p_batch_id
			                AND psd.property_bk IS NOT NULL
                            AND psd.delinquency_tax_year IS NOT NULL
                            AND psd.event_batch_id IS NULL  /* Only for Loan */
                            AND psd.county_id = 0 
                            AND c.""Id"" IS NULL;
                   ...
                   /* Update staging data with new county ids */
	                UPDATE stage.synergy_source_data psd SET  
		                county_id = c.""Id"",
		                property_state_id = s.""Id""
	                FROM main.""County"" c
    	                INNER JOIN main.""State"" s ON s.""Id"" = c.""StateId""
                    WHERE (LOWER(psd.property_state) = LOWER(s.""Abbreviation"") AND LOWER(c.""Name"") = LOWER(psd.county_jurisdiction))
    	                AND psd.batch_id = p_batch_id
		                AND psd.property_bk IS NOT NULL
                        AND psd.delinquency_tax_year IS NOT NULL
                        AND psd.event_batch_id IS NULL  /* Only for Loan */
                        AND psd.county_id = 0;
                    ...
                    RETURN QUERY
                    SELECT 0::numeric, COALESCE(p_icnt, 0);		
