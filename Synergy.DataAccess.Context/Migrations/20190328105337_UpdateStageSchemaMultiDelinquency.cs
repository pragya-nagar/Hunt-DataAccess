...
{
    public partial class UpdateStageSchemaMultiDelinquency : Migration
    {
        protected override void Up(MigrationBuilder migrationBuilder)
        {
            string sqlStageScript = @"/* create needed extensions  */
                ...
                BEGIN
	                ...
	                (
		                SELECT DISTINCT ON (d.""Id"")  
			                ...
			                ,sum(amount_due) OVER (ORDER BY d.""Id"", delinquency_tax_year) AS amount
			                ,ru_amount
			                ,ru_ltv_percent
			                ,ltv_percent
		                FROM stage.synergy_source_data psd
	                ...
	                (
		                SELECT DISTINCT ON (p.""Id"")
			                 p.""Id"" as property_id
			                ,psd.delinquency_tax_year
			                ,sum(amount_due) OVER (ORDER BY p.""Id"", psd.delinquency_tax_year  ) AS amount
			                ,ru_amount
			                ,ru_ltv_percent
			                ,ltv_percent
		                FROM stage.synergy_source_data psd
	                ...
	                SELECT uuid_generate_v4() as id, d.property_id, d.delinquency_tax_year, d.amount, d.ltv_percent, d.ru_amount, d.ru_ltv_percent, LOCALTIMESTAMP, p_system_user_id, LOCALTIMESTAMP, p_system_user_id
	                FROM d
	                ...
	                (
		                SELECT DISTINCT ON (p.""Id"")
			                ...
			                ,e.""Id"" as event_id
			                ,delinquency_tax_year
			                ,sum(amount_due) OVER (ORDER BY p.""Id"", e.""Id"", delinquency_tax_year) AS amount
			                ,ru_amount
			                ,ru_ltv_percent
			                ,ltv_percent
		                FROM stage.synergy_source_data psd
	                ...
	                ,ins AS (
		                ...
		                SELECT uuid_generate_v4() as id, d.property_id, d.event_id, d.delinquency_tax_year, d.amount, d.ltv_percent, d.ru_amount, d.ru_ltv_percent, LOCALTIMESTAMP, p_system_user_id, LOCALTIMESTAMP, p_system_user_id
		                FROM d
		                WHERE NOT EXISTS ( SELECT 1 FROM main.""Delinquency"" dl 
							                WHERE dl.""PropertyId"" = d.property_id 
								                AND dl.""DelinquencyTaxYear"" = d.delinquency_tax_year
